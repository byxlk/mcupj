...
#if PHASE_OUTPUT_MODE
    if(iSecondCounter % 200 == 0) {
        PWR_PHASE_PIN_A  = ~PWR_PHASE_PIN_A;
        pha_tick = iSecondCounter;
    }
    if(iSecondCounter == pha_tick + 133) {
        PWR_PHASE_PIN_B = ~PWR_PHASE_PIN_B;
    }
    
    if(iSecondCounter == pha_tick + 267) {
        PWR_PHASE_PIN_C = ~PWR_PHASE_PIN_C;
    }
#else
    checkPhaseSeqALost();
    checkPhaseSeqBLost();
